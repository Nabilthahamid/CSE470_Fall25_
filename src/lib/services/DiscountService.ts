// SERVICE: Discount & Coupon Management
...
export class DiscountService {
	...
	async validateDiscount(
		...
		if (discount.applicable_to === 'products' && discount.applicable_ids && productIds) {
			const hasApplicableProduct = productIds.some((id) =>
				discount.applicable_ids?.includes(id)
			);
	...
	async createDiscount(discount: CreateDiscountDTO): Promise<Discount> {
		...
		const { data, error } = await supabase
			.from('discounts')
			.insert({ ...discount, used_count: 0, created_at: new Date().toISOString() })
			.select()
			.single();
	 ...
	 * Update discount
	 */
	async updateDiscount(id: string, discount: UpdateDiscountDTO): Promise<Discount> {
		if (discount.code) {
			discount.code = discount.code.toUpperCase();
		}
	 ...
	 */
	async getDiscountAnalytics(startDate?: string, endDate?: string): Promise<DiscountAnalytics> {
		const discounts = await this.getAllDiscounts();
		...
		// Calculate revenue impact (simplified - assume discounts reduce revenue)
		...
		// Top discounts
		const discountUsageMap = new Map<string, { usage: number; totalAmount: number }>();
		allUsage.forEach((u) => {
		...
		const topDiscounts = Array.from(discountUsageMap.entries())
			.map(([discountId, data]) => {
		...
		// By month
		const monthlyMap = new Map<string, { usage: number; totalAmount: number }>();
		allUsage.forEach((u) => {
	 ...
	 * Get discount usage
	 */
	async getDiscountUsage(startDate?: string, endDate?: string): Promise<DiscountUsage[]> {
		try {
