  ...
  try {
    ...
    const timeComparisons = comparePeriod ? {
      currentPeriod: {
        ...
        averageOrderValue: periodOrders.length > 0 ? periodOrders.reduce((sum, o) => sum + o.total_amount, 0) / periodOrders.length : 0,
      ...
      previousPeriod: {
        ...
        averageOrderValue: previousPeriodOrders.length > 0 ? previousPeriodOrders.reduce((sum, o) => sum + o.total_amount, 0) / previousPeriodOrders.length : 0,
...
function calculateConversionTrends(orders, users, period) {
...
function calculateAOVTrends(orders, period) {
  if (orders.length === 0) return [];
  const trends = {};
  orders.forEach((order) => {
    if (!order.created_at) return;
...
function calculateRetentionMetrics(allOrders, allUsers, periodStart, periodEnd) {
  const periodOrders = allOrders.filter((order) => {
    if (!order.created_at) return false;
    const orderDate = new Date(order.created_at);
    return orderDate >= periodStart && orderDate <= periodEnd;
  ...
  const daysSincePeriodEnd = (now2, date) => Math.floor((now2.getTime() - date.getTime()) / (1e3 * 60 * 60 * 24));
  const now = /* @__PURE__ */ new Date();
  let champions = 0;
  let loyal = 0;
  let atRisk = 0;
  ...
  customerStats.forEach((stats) => {
    ...
    if (stats.orderCount >= 3 && avgOrderValue >= 1e3 && daysSinceLastOrder <= 30) {
      champions++;
...
function calculateGrossMarginAnalysis(sales, products) {
  ...
  const marginByCategory = Array.from(categoryMap.entries()).map(([category, data]) => ({
    category,
  ...
  const marginByProduct = Array.from(productMap.values()).map((data) => ({
